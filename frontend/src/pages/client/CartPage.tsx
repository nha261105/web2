...
import { ArrowRight, ShieldCheck, Tag, Trash2 } from "lucide-react";
...
export default function CartPage(){
    ...
    return <>
        <div className="bg-gray-50 min-h-screen flex flex-col items-center px-7 pt-6 pb-20">
            <div className="w-full max-w-250 flex flex-col gap-5">
                <MyNavigateLink items={[ 
                    { text: "Home", link: "/"},
                    { text: "Shopping Cart" } 
                ]}/>
                <div className="w-full flex flex-row items-center gap-2">
                    <div className="text-xl text-black font-semibold">Shopping Cart</div>
                    <div className="text-lg text-gray-500 font-normal">({products.length} items)</div>
                </div>
                <div className="w-full flex flex-row justify-between gap-10 flex-wrap">
                    <div className="w-full lg:flex-2 flex flex-col gap-3">
                    {
                        products.map((item) => (
                            <div className="flex flex-row border-gray-300 bg-white border p-5 rounded-lg gap-3">
                                <img src={item.img} alt="meme" className="w-20 h-20 rounded-lg cursor-pointer"/>
                                <div className="w-full flex flex-col justify-between">
                                    <div className="w-full flex flex-row justify-between">
                                        <div className="flex flex-col gap-0.5">
                                            <div className="text-sm font-semibold hover:cursor-pointer hover:text-blue-700">MacBook Pro 16-inch M3 Max</div>
                                            <div className="text-xs font-medium text-gray-500">$89/day</div>
                                        </div>
                                        <Trash2 className="w-4.5 text-gray-400 hover:cursor-pointer hover:text-red-400"/>
                                    </div>
                                    <div className="w-full flex flex-row justify-between">
                                        ...
                                        <div className="text-sm font-bold">$3204</div>
                                    </div>
                                </div>
                            </div>
                        ))
                        ...
                        <div className="w-full flex flex-col border-gray-300 bg-white border p-5 rounded-lg gap-3">
                            ...
                            <div className="w-full flex flex-row justify-between items-center gap-3">
                                <MyInputText defaultValue="" placeholder="Enter promo code (try RENT10)"/>
                                <MyButton text="Apply"/>
                            </div>
                            <div className="text-sm text-gray-500">Try: RENT10, FIRST15, SAVE20</div>
                        </div>
                    </div>
                    <div className="flex flex-col flex-1">
                        <div className="w-full flex flex-col border-gray-300 bg-white border p-5 rounded-lg gap-3">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <MyButton text="Proceed to Checkout" classname="flex-1 py-2" icon={<ArrowRight size={20} strokeWidth="2.25px" className="text-white"/>}/>
                                <MyHref text="← Continue Shopping" src="/"/>
                            </div>
                            <hr />
                            <div className="flex flex-row items-center gap-2">
                                <ShieldCheck size={17} className="text-green-700"/>
                                <div className="text-xs text-gray-500">Secure checkout — SSL encrypted</div>
                            </div>
                        </div>  
                    </div>
                </div>
